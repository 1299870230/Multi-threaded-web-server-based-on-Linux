...
class threadpool{
    ...
    private:
        ...
        pthread_t *m_threads;		//描述线程池的数组，其大小为m_thread_number，存放线程池中线程
...
threadpool<T>::threadpool(int thread_number,int max_requests):m_thread_number(thread_number),m_max_requests(max_requests),m_stop(false),m_threads(NULL){
    if(thread_number<=0||max_requests<=0)
        throw std::exception();
    m_threads=new pthread_t[m_thread_number];//创建一个存放线程池中线程的数组
    if(!m_threads)
        throw std::exception();
    ...
    for(int i=0;i<thread_number;++i)
    {
		...
		//pthread_create成功，返回0。
		//创建线程
        if(pthread_create(m_threads+i,NULL,worker,this)!=0){
            delete [] m_threads;
            throw std::exception();
        }
		//pthread_detach成功，返回0。
